...
def validate_hybrid(opt, tb_logger, vid_data_loader, text_data_loader, model, measure='cosine'):
    # compute the encoding for all the validation video and captions
    model.val_start()
    ...
    t2v_all_errors_2 = evaluation.cal_error(video_embs, cap_trans_embs, measure)
    v2t_gt, t2v_gt = metrics.get_gt(video_ids, caption_ids)

    t2v_all_errors_1 = norm_score(t2v_all_errors_1)
    t2v_all_errors_2 = norm_score(t2v_all_errors_2)
    print('-----',opt.sim_ratio)
    t2v_all_errors = opt.sim_ratio * t2v_all_errors_1 + (1-opt.sim_ratio) * t2v_all_errors_2
    _, (t2v_r1, t2v_r5, t2v_r10, t2v_medr, t2v_meanr, t2v_map_score) = cal_perf(t2v_all_errors, v2t_gt, t2v_gt,
