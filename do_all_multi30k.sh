...
text_layer=1
frozen=frozen
data_type=google_enc2de # google_enc2fr # google_enc2cs
glr=1e-3
scale=0.001
...
framework=Full_multi30k_${frozen}_task_${task}/data_type_$data_type/tri_alpha_$tri_alpha/dtl_beta_$dtl_beta/l1_gama_$l1_gama/back_w_$back_w/video_layer_$video_layer/text_layer_$text_layer/text_num_attention_$text_num_attention/text_hidden_size_$text_hidden_size/vido_pooling_$video_pooling/text_pooling_$text_pooling\
/layer_list_$layer_list/glr_$glr/scale_$scale/disc_type_$disc_type/momentum_$momentum/optim_$optim
#framework=test

img_path=multi30k/flickr30k-images
img_encoder=clip #resnet_152 # clip
...
CUDA_VISIBLE_DEVICES=$gpu python trainer_img.py --rootpath $rootpath --overwrite $overwrite --max_violation --text_norm --visual_norm \
                                            ...
                                            --img_path $img_path --img_encoder $img_encoder --task $task
