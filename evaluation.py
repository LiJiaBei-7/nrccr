...
# encode text
def encode_text_hybrid(encoder, data_loader, return_ids=True):
    """Encode all videos and captions loadable by `data_loader`
    """
    # numpy array to keep all the embeddings
    embeddings = None
    embeddings_trans = None
    ids = ['']*len(data_loader.dataset)
    pbar = Progbar(len(data_loader.dataset))
    for i, (datas, idxs, data_ids) in enumerate(data_loader):

        # compute the embeddings
        emb, emb_trans = encoder(datas)

        # initialize the numpy arrays given the size of the embeddings
        if embeddings is None:
            embeddings = np.zeros((len(data_loader.dataset), emb.size(1)))
            embeddings_trans = np.zeros(embeddings.shape)

        # preserve the embeddings by copying from gpu and converting to numpy
        embeddings[idxs] = emb.data.cpu().numpy().copy()
        embeddings_trans[idxs] = emb_trans.data.cpu().numpy().copy()
    ...
    if return_ids == True:
        return embeddings, embeddings_trans, ids,
    else:
        return embeddings, embeddings_trans
